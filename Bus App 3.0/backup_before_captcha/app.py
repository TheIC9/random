...
def init_db():
    ...
    try:
        ...
        schedules = [
            {"bus_id": bus_id, "departure_date": today, "departure_time": "08:00", "available_seats": 40, "status": "active"},
            {"bus_id": bus_id, "departure_date": today, "departure_time": "14:00", "available_seats": 40, "status": "active"},
            {"bus_id": bus_id, "departure_date": today, "departure_time": "18:00", "available_seats": 40, "status": "active"},
            {"bus_id": bus_id, "departure_date": tomorrow, "departure_time": "08:00", "available_seats": 40, "status": "active"}
        ]
...
def dashboard():
    ...
    try:
        ...
        # Flatten bookings data and filter by date
        bookings = []
...
def confirm_booking():
    ...
    try:
        ...
        schedule = supabase.table('schedules').select("*").eq('schedule_id', schedule_id).execute().data[0]
...
def manage_buses():
    ...
    try:
        ...
        return render_template('manage_buses.html', buses=buses_result.data)
    except Exception as e:
        flash(f"Error: {e}")
...
def cancel_schedule(schedule_id):
    ...
    try:
        result = supabase.table('schedules').select('*, buses(bus_number, total_seats)').execute()
        buses_data = []
        for b in result.data:
            buses_data.append({
                ...
                'total_seats': b['buses']['total_seats'],
                'available_seats': b['available_seats']
            })

        return render_template('manage_buses.html', buses=buses_data)
    except Exception as e:
        flash(f"Error loading buses: {e}")
        return render_template('manage_buses.html', buses=[])
...
def all_bookings():
    ...
    try:
        ...
        ).eq('status', 'confirmed').order('schedules(departure_date)').execute()
        
        return render_template('all_bookings.html', bookings=bookings_result.data)
    except Exception as e:
        flash(f"Error: {e}")
