...
def edit_schedule(schedule_id):
    ...
    if request.method == 'POST':
        ...
        departure_datetime = request.form.get('departure_time', '').strip()
        total_seats_str = request.form.get('total_seats', '').strip()
        
        # Validate inputs
        if not bus_number or not departure_datetime or not total_seats_str:
            flash('All fields are required')
        ...
        try:
            ...
            flash('Invalid seat number. Please enter a valid number.')
            return redirect(url_for('edit_schedule', schedule_id=schedule_id))
        ...
        try:
            ...
            booking_count = supabase.table('bookings').select("*", count='exact').eq('schedule_id', schedule_id).eq('status', 'confirmed').execute()
            booked_seats = booking_count.count if booking_count.count else 0
            ...
            if available_seats < 0:
                flash(f"Cannot set capacity to {total_seats}. Already {booked_seats} seats are booked. Minimum capacity must be {booked_seats}.")
                return redirect(url_for('edit_schedule', schedule_id=schedule_id))
            ...
            activity_data = {
                ...
                "description": f"Updated schedule {schedule_id} (kept {booked_seats} existing bookings)"
            }
            supabase.table('admin_activity_log').insert(activity_data).execute()
            
            flash(f"Schedule updated successfully. {booked_seats} existing bookings preserved.")
            return redirect(url_for('admin_panel'))
        except Exception as e:
    ...
    try:
        ...
        if schedule_result.data:
            ...
            # Calculate total capacity from available + booked
            total_capacity = s['available_seats'] + booked_seats
            # new_capacity = s['available_seats'] - booked_seats
            bus = {
                'schedule_id': s['schedule_id'],
                'bus_number': s['buses']['bus_number'],
                'route': s['buses']['routes']['route_name'],
                'departure_time': f"{s['departure_date']}T{s['departure_time']}",
                'total_seats': total_capacity,  # Show actual capacity, not just available
                'booked_seats': booked_seats,    # Pass this to template
                'available_seats': s['available_seats']
            }
