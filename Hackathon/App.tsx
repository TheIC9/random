...
function ResultCards({ result, lang }: { result: PipelineResult; lang: string }) {
  ...
  // Build minimal diff from snippet → correctedCode
  const diffLines = [
  ...
  return (
    <div className="slide-in" style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      <div className="result-card">
        ...
        {result.line > 0 && (
          <div className="error-line">Line {result.line} · {lang}</div>
        )}
      </div>
      ...
      <div className="result-card">
        <div className="result-card-header">
          <span className="result-card-title">🔍 Root Cause</span>
        </div>
      ...
      <div className="result-card">
        <div className="result-card-header">
          <span className="result-card-title">💡 Explanation</span>
        </div>
        <div className="tabs">
      ...
      <div className="result-card">
        <div className="result-card-header">
          <span className="result-card-title">🔧 Suggested Fix</span>
        </div>
        <div style={{ fontSize: 12, color: "var(--muted2)", marginBottom: 8 }}>{result.description}</div>
        <div className="fix-code" id="fixCode">
      ...
      <div className="result-card">
        <div className="result-card-header">
          <span className="result-card-title">📋 Diff View</span>
        </div>
...
// ─── Pipeline bar (file mode) ─────────────────────────────────────────────────

const PIPE_STAGES: { id: string; label: string }[] = [
  ...
  return (
    <div className="pipeline-bar">
      {PIPE_STAGES.map((s, i) => {
        ...
        const myIdx = i;
        const cls =
          current === "done" ? "pipe-stage done"
          : myIdx < idx ? "pipe-stage done"
          : myIdx === idx ? "pipe-stage active"
          : "pipe-stage";
        return (
          <span key={s.id}>
...
function FileMode() {
  ...
  const [overlayStage, setOverlayStage] = useState("STAGE 1/3 — PARSING");
  ...
  return (
    <div className="main-area">
      ...
      <div className="panel left-panel-file">
        ...
        <div className="controls-bar">
          <select value={lang} onChange={(e) => setLang(e.target.value)}>
            {["Python","JavaScript","TypeScript","Java","C++","Go","Rust","Ruby"].map(l => (
          ...
          <button
            className={`btn-analyze${showOverlay ? " loading" : ""}`}
            style={{ marginLeft: "auto" }}
            onClick={analyze}
            disabled={showOverlay || !code.trim()}
          >
      ...
      {/* RIGHT: results */}
      <div className="panel right-panel" style={{ width: "100%" }}>
        <PipelineBar current={stage} />
        <div id="fileResults" className="right-scroll" style={{ padding: 14 }}>
          {stage === "idle" && (
          ...
          {stage === "error" && (
            <div className="result-card" style={{ borderColor: "rgba(255,77,106,.3)" }}>
              <div className="result-card-title" style={{ color: "var(--red)", marginBottom: 8 }}>
                ⚠ Pipeline Error
              </div>
              <div className="explanation-text">{error}</div>
              <div className="explanation-text" style={{ marginTop: 8, fontSize: 11 }}>
                Make sure <code style={{ color: "var(--blue)" }}>uvicorn main:app --reload</code> is running on port 8000.
              </div>
            </div>
          )}
...
function RepoMode() {
  ...
  const scanAll = async () => {
    ...
    const stageLabels = ["Parsing files...", "Running AI analysis...", "Generating fixes..."];
    for (let i = 0; i < stageLabels.length; i++) {
      setScanStage(i);
      await sleep(700 + Math.random() * 400);
    }

    // Call API for each file sequentially
    const updated = [...files];
    for (let i = 0; i < updated.length; i++) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          f.status = "ok"; // treat as clean on failure
        }
      } catch {
        f.status = "pending";
      }
    ...
    setScanDone(true);
    if (selected) {
      const refreshed = updated.find((f) => f.name === selected.name);
  ...
  return (
    <div className="main-area">
        ...
        <div className="toc-list">
          {Object.keys(folders).length === 0 ? (
            <div style={{ padding: "20px", textAlign: "center", color: "var(--muted)", fontSize: 12 }}>
              {files.length === 0 ? "Upload files to begin" : "No files match"}
            </div>
          ) : Object.entries(folders).map(([folder, fls]) => (
            <div key={folder}>
              <div className="toc-folder">📁 <span>{folder}</span></div>
              {fls.map((f) => {
                const statusCls = f.status === "ok" ? "status-ok" : f.status === "error" ? "status-err" : "status-pending";
                return (
                  <div
                    key={f.name}
                  ...
                  >
                    <div className={`file-status ${statusCls}`} />
                    <div className="file-name">{f.name}</div>
                    {f.severity && (
                      <div className={`severity-badge ${f.severity === "High" || f.severity === "Critical" ? "sev-high" : "sev-med"}`} style={{ fontSize: 9, padding: "1px 5px" }}>
                        {f.severity[0]}
                      </div>
                    )}
                  </div>
      ...
      <div className="panel center-panel">
        ...
        <div className="pipeline-graph-wrap">
          ...
          <div className="pipeline-graph">
            {PG_NODES.map((n, i) => {
              const cls = i === activeNode ? "pg-node active-node"
                : scanDone && i <= activeNode ? "pg-node ok-node"
                : "pg-node";
              return (
                <span key={i} style={{ display: "contents" }}>
                  <div className={cls} onClick={() => setActiveNode(i)}>
        ...
        <div className="editor-wrap" style={{ position: "relative" }}>
          {!selected ? (
            <div className="placeholder">
              <div className="placeholder-icon">📂</div>
              <span>Select a file from the TOC</span>
            </div>
          ) : (
            <textarea
              id="repoEditor" spellCheck={false}
          ...
          {scanning && (
            <div className="analyzing-overlay show">
              ...
              <div className="analyzing-text">
                {["Parsing files...", "Running AI analysis...", "Generating fixes..."][scanStage]}
              </div>
        ...
        <div className="controls-bar">
          ...
          >
            ...
            <span style={{ fontSize: 12, color: "var(--muted2)" }}>
              {files.length > 0 ? `${files.length} file(s) loaded` : "Upload folder / drop files"}
            </span>
      ...
      <div className="panel right-panel">
        <div className="pipeline-bar">
          {[{ id: "rps-parse", label: "Parse" }, { id: "rps-scan", label: "Scan" }, { id: "rps-fix", label: "Fix" }].map((s, i) => {
            const cls = scanning && i === scanStage ? "pipe-stage active"
              : scanning && i < scanStage ? "pipe-stage done"
              : scanDone ? "pipe-stage done"
              : "pipe-stage";
            return (
              <span key={s.id}>
                <span className={cls}><span className="pipe-dot" />{s.label}</span>
        ...
        <div className="right-scroll" style={{ padding: 14 }}>
          {!selected && (
            <div className="placeholder">
              <div className="placeholder-icon">🧬</div>
              <span>Select a file to view analysis</span>
            </div>
          )}
          {selected && selected.status === "pending" && (
            <div className="placeholder" style={{ padding: "40px 0" }}>
              <div className="placeholder-icon">⏳</div>
              <span>Run "Scan All" to analyze</span>
            </div>
          )}
            ...
            <div>
              <div className="result-card" style={{ borderColor: "rgba(46,229,157,.2)" }}>
                ...
                <div style={{ fontSize: 12, color: "var(--muted2)" }}>
                  This file passed all static analysis checks. No errors or warnings detected.
                </div>
              </div>
              <ConfBar score={0.98} active />
            </div>
          )}
          {selected?.result && selected.status === "error" && (
            <ResultCards result={selected.result} lang={selected.result.language} />
          )}
          {scanDone && (
            <div style={{ padding: "12px 0", textAlign: "center", fontSize: 11, color: "var(--muted2)" }}>
              ✓ Scan complete — {errorCount} issue(s) found
            </div>
          )}
        </div>
      </div>
    ...
    <>
      ...
      <style>{`
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        html,body,#root{width:100%;height:100%;overflow:hidden;background:var(--bg);color:var(--text);font-family:'DM Sans',sans-serif;}
        ...
        #landing{position:fixed;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;
          background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(91,140,255,0.07) 0%,transparent 70%),
                     radial-gradient(ellipse 60% 40% at 80% 100%,rgba(157,92,255,0.05) 0%,transparent 60%),var(--bg);
        ...
        .grid-bg{position:absolute;inset:0;
          ...
          background-size:48px 48px;mask-image:radial-gradient(ellipse 80% 80% at 50% 50%,black 20%,transparent 100%);}
        .landing-logo{display:flex;align-items:center;gap:16px;margin-bottom:64px;animation:fadeUp .8s cubic-bezier(.16,1,.3,1) both;}
        .landing-logo-text{font-family:'Syne',sans-serif;font-size:32px;font-weight:800;
          background:linear-gradient(135deg,#fff 30%,var(--blue));-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
        .landing-tagline{font-size:13px;letter-spacing:.2em;text-transform:uppercase;color:var(--muted2);
          margin-bottom:56px;animation:fadeUp .8s .1s cubic-bezier(.16,1,.3,1) both;}
        .landing-cards{display:flex;gap:20px;animation:fadeUp .8s .2s cubic-bezier(.16,1,.3,1) both;}
        .option-card{width:280px;padding:32px 28px;border-radius:16px;cursor:pointer;position:relative;overflow:hidden;
          background:var(--panel);border:1px solid var(--border2);transition:transform .25s,border-color .25s,box-shadow .25s;}
        ...
        .card-icon{width:52px;height:52px;border-radius:12px;display:flex;align-items:center;justify-content:center;
          background:linear-gradient(135deg,rgba(91,140,255,0.15),rgba(157,92,255,0.08));border:1px solid rgba(91,140,255,0.2);margin-bottom:20px;font-size:22px;}
        ...
        /* APP */
        #appShell{position:fixed;inset:0;display:flex;flex-direction:column;transition:opacity .5s .1s;}
        .header{height:52px;display:flex;align-items:center;justify-content:space-between;
          padding:0 20px;border-bottom:1px solid var(--border2);background:rgba(8,12,16,.9);
          backdrop-filter:blur(12px);z-index:50;flex-shrink:0;}
        .header-logo{display:flex;align-items:center;gap:10px;cursor:pointer;}
        .header-logo-text{font-family:'Syne',sans-serif;font-weight:800;font-size:16px;
          background:linear-gradient(135deg,#fff,var(--blue));-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
        .mode-badge{padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;letter-spacing:.05em;
          background:linear-gradient(135deg,rgba(91,140,255,0.15),rgba(157,92,255,0.1));
          border:1px solid rgba(91,140,255,0.25);color:var(--blue);font-family:'JetBrains Mono',monospace;}
        .header-actions{display:flex;align-items:center;gap:8px;}
        .btn-sm{padding:5px 14px;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer;border:none;
          background:rgba(91,140,255,0.1);border:1px solid rgba(91,140,255,0.2);color:var(--blue);
        ...
        .left-panel-file{width:420px;flex-shrink:0;}
        .left-panel-repo{width:220px;flex-shrink:0;}
        .center-panel{flex:1;min-width:0;}
        .right-panel{width:360px;flex-shrink:0;border-right:none;border-left:1px solid var(--border2);}
        .panel-header{padding:12px 16px;border-bottom:1px solid var(--border2);display:flex;align-items:center;gap:8px;
          font-size:11px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:var(--muted2);flex-shrink:0;}
        .panel-header-dot{width:6px;height:6px;border-radius:50%;background:var(--blue);box-shadow:0 0 8px var(--glow-blue);}
        .editor-wrap{flex:1;position:relative;overflow:hidden;}
        #codeEditor,#repoEditor{width:100%;height:100%;border:none;outline:none;background:transparent;resize:none;
          font-family:'JetBrains Mono',monospace;font-size:13px;line-height:1.7;color:#C9D1D9;padding:16px;tab-size:2;overflow:auto;}
        .editor-gutter{position:absolute;left:0;top:0;bottom:0;width:40px;background:rgba(0,0,0,.2);
          display:flex;flex-direction:column;padding:16px 0;pointer-events:none;
          font-family:'JetBrains Mono',monospace;font-size:13px;line-height:1.7;
          color:var(--muted);text-align:right;padding-right:8px;overflow:hidden;}
        ...
        .btn-analyze{margin-left:auto;padding:6px 18px;border-radius:8px;border:none;cursor:pointer;
          background:linear-gradient(135deg,var(--blue),var(--purple));color:#fff;font-size:12px;font-weight:600;
          font-family:'Syne',sans-serif;letter-spacing:.05em;position:relative;overflow:hidden;transition:box-shadow .2s,transform .15s;}
        ...
        .pipe-stage{display:flex;align-items:center;gap:6px;padding:4px 10px;border-radius:20px;font-size:11px;
          font-family:'JetBrains Mono',monospace;color:var(--muted2);border:1px solid transparent;transition:all .3s;white-space:nowrap;}
        ...
        .fix-code{background:var(--bg);border:1px solid var(--border2);border-radius:7px;padding:12px;
          font-family:'JetBrains Mono',monospace;font-size:12px;color:#A8D8A8;line-height:1.7;overflow-x:auto;position:relative;}
        .copy-btn{position:absolute;top:8px;right:8px;padding:3px 8px;border-radius:5px;
          background:rgba(91,140,255,.12);border:1px solid rgba(91,140,255,.2);color:var(--blue);
          font-size:10px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all .2s;}
        ...
        .toc-search input{width:100%;background:var(--bg3);border:1px solid var(--border2);border-radius:6px;padding:5px 10px;font-size:12px;color:var(--text);outline:none;font-family:'DM Sans',sans-serif;}
        ...
        .filter-chip{padding:3px 8px;border-radius:12px;font-size:10px;font-weight:600;cursor:pointer;border:1px solid var(--border2);color:var(--muted2);background:transparent;transition:all .2s;font-family:'DM Sans',sans-serif;}
        ...
        .toc-folder{padding:6px 12px;font-size:11px;color:var(--muted);display:flex;align-items:center;gap:6px;cursor:pointer;letter-spacing:.05em;}
        ...
        .pg-node{display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px 12px;border-radius:8px;
          cursor:pointer;transition:all .25s;border:1px solid transparent;min-width:60px;}
        ...
        .pg-node.error-node{background:rgba(255,77,106,.08);border-color:rgba(255,77,106,.2);}
        ...
        .analyzing-overlay{position:absolute;inset:0;background:rgba(8,12,16,.85);backdrop-filter:blur(6px);
        ...
        .upload-zone{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;
          border:2px dashed rgba(91,140,255,.15);border-radius:10px;margin:14px;cursor:pointer;transition:all .25s;}
      ...
      <div id="landing" className={landingHidden ? "hidden" : ""}>
        ...
        <div className="landing-cards">
          ...
          <div className="option-card" onClick={() => enterMode("repo")}>
            ...
            <div className="card-desc">Upload an entire project or repository. Get a full pipeline scan across all files with dependency-aware analysis.</div>
      ...
      {/* APP SHELL */}
      {mode !== "landing" && (
        <div id="appShell" style={{ display: "flex", flexDirection: "column" }}>
